...
//uniform vec3			uSphereGlow = vec3( 0.6, 0.15, 0.15 );
uniform vec3			uSphereGlow = vec3( 0.6, 0.6, 0.6 );
...
void main()
{
	...
	vec3	sphereGlow   = uSphereGlow;
	vec3	indirectGlow = vec3(
                clamp( dot( uSphereGlow.r * normalize(vModelNormal), -normalize(vModelPosition.xyz) ), 0.0, 0.55 ),
                clamp( dot( uSphereGlow.g * normalize(vModelNormal), -normalize(vModelPosition.xyz) ), 0.0, 0.55 ),
                clamp( dot( uSphereGlow.b * normalize(vModelNormal), -normalize(vModelPosition.xyz) ), 0.0, 0.55 )
            );
    vec3	A = mix( sphereGlow, indirectGlow, float(uIsTeapot) ) + uSphereGlow/10.0;//vec3( 0.07, 0.05, 0.1 );
	// Diffuse factor
